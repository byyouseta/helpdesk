        ...
        <div class="card mb-3">
            <div class="card-header">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="Unit Name" aria-label="Recipient's username"
                        aria-describedby="button-addon2" wire:model="unit_name" />
                    <button class="btn {{ $tombolMulti == 'Save' ? 'btn-primary' : 'btn-warning' }} btn-sm"
                        type="button" id="button-addon2"
                        @if ($tombolMulti == 'Save') wire:click="simpan()"
                        @else
wire:click="update()" @endif>{{ $tombolMulti }}
                    </button>
                    @if ($tombolMulti != 'Save')
